...
function RewardItem({ redemption }: { redemption: RedemptionWithReward }) {
    ...
    return (
        <>
            <Card
                className="overflow-hidden flex flex-col h-full border-l-4 border-l-primary/50 shadow-sm hover:shadow-md transition-shadow cursor-pointer"
            ...
            <Dialog open={detailsOpen} onOpenChange={setDetailsOpen}>
                <DialogContent className="sm:max-w-md max-h-[90vh] overflow-y-auto">
                    <DialogHeader>
                        <DialogTitle>{rewards.name}</DialogTitle>
                    </DialogHeader>
                    <div className="space-y-4">
                        <div className="aspect-video bg-muted relative rounded-md overflow-hidden">
                            {rewards.image_url ? (
                                <img src={rewards.image_url} alt={rewards.name} className="w-full h-full object-cover" />
                            ) : (
                            ...
                            <div className="absolute top-2 right-2">
                                <Badge variant={status === 'redeemed' ? 'secondary' : 'default'} className="capitalize bg-white/90 text-black shadow-sm">
                                    {(status === 'pending' || status === 'approved') ? 'Available' : status}
                                </Badge>
                            </div>
                        </div>
                        ...
                        <div className="text-xs text-muted-foreground space-y-1 bg-muted/50 p-3 rounded-md">
                            <div className="flex justify-between">
                                <span>Claimed:</span>
                                <span className="font-medium text-foreground">{new Date(created_at).toLocaleDateString()}</span>
                            </div>
                            <div className="flex justify-between">
                                <span>Points Spent:</span>
                                <span className="font-medium text-foreground">{points_spent} pts</span>
                            </div>
                            {(status === 'pending' || status === 'approved') && redemption.expires_at && (
                                <div className="flex justify-between items-center text-amber-600 dark:text-amber-400 mt-2 pt-2 border-t">
                                    <span className="flex items-center gap-1"><Clock className="w-3 h-3" /> Expiration:</span>
                        ...
                        <Button className="w-full" variant="outline" onClick={() => setDetailsOpen(false)}>
                            Close
                        </Button>
                    </div>
                </DialogContent>
